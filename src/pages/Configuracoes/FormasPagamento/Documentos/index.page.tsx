...
export default function Documentos() {
  ...
  async function handleSearchUsers() {
    ...
    if (searchName || searchPayment || searchStatus) {
      await api.get(`/api/Configuracoes/SearchTipoDocumento?${searchPayment && "&PaymentTypeId=" + searchPayment}${searchName && 'name=' + searchName}${searchStatus != "3" && searchStatus ? "&isActive=" + (searchStatus === "1" ? true : false) : ''}${searchStatus != "1" && searchStatus ? "&isDeleted=" + (searchStatus === "3" ? true : false) : ''}`)
        .then(response => {
  ...
  function handleOpenModalOrActivateAction(typeModal: 'view' | 'create' | 'update' | 'search' | 'disable' | 'delete' | 'reactivate' | 'restore', row?: createDataProps) {
    ...
    if (typeModal === 'create') {
      ...
      reset({ id: row?.id, name: row?.name, NumberFormat: row?.paymentTypeId.toString() });
      setOpen(true);
      setLoadingAction(false);
      ...
      api.delete(`/api/Configuracoes/DeletaTiposDocumentos/${row?.id}`)
        .then((response) => {
      ...
      api.delete(`/api/Configuracoes/DesabilitaTiposDocumentos/${row?.id}`)
        .then((response) => {
      ...
      api.put(`/api/Configuracoes/HabilitarTiposDocumentos/${row?.id}`)
        .then((response) => {
  ...
  async function handleActionDocumentos(data: DocumentosFormData) {
    ...
    if (changeTypeModal === 'create') {
      await api.post(`/api/Configuracoes/SalvarTiposDocumentos`, {
        name,
      ...
      await api.put(`/api/Configuracoes/AtualizaTiposDocumentos`, {
        id,
        name,
        paymentTypeId: Number(NumberFormat),
        isDeleted: false,
    ...
    <>
      ...
      <Configuracoes>
        <Container>
          <Header>
            <TitleBox>
              <span>Tipos de documentos validos</span>
            </TitleBox>
            <ActionsHeader>
              ...
              <TableBody>
                {(rowsPerPage > 0
                  ...
                  <TableRow key={row.id}>
                    ...
                    <TableCell component="th" scope="row">
                      {row.siengeId + " - " + row.paymentTypeName}
                    </TableCell>
                      ...
                      <DocumentosUpdateOrCreate>
                        ...
                        <Label size={"50"}>
                          ...
                          <select style={inputStyle} {...register("NumberFormat")} disabled={changeTypeModal === 'view' && true}>
                            <option value="" selected>Formas de pagamentos</option>
                            {
